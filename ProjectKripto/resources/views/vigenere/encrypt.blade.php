...
<body class="text-white min-h-screen flex items-center justify-center font-sans bg-cover bg-center" style="background-image: url('/icons/kripto.png');">
    ...
    <div class="bg-white text-gray-900 p-8 rounded-xl shadow-2xl w-full max-w-lg">
        <h2 class="text-3xl font-bold mb-6 text-center text-indigo-700">🔑 Enkripsi Vigenère</h2>

        <form action="{{ route('vigenere.process') }}" method="POST" class="space-y-5">
            @csrf
            ...
            <div>
                ...
                <textarea id="plaintext" name="plaintext" rows="3"
                    ...
                    placeholder="Masukkan teks yang ingin dienkripsi...">{{ old('plaintext') }}</textarea>
            </div>

            <!-- Key -->
            <div>
                <label for="key" class="block font-semibold mb-2">Kunci</label>
                <input type="text" id="key" name="key"
                    class="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500"
                    placeholder="Masukkan kunci enkripsi (huruf saja)..."
                    value="{{ old('key') }}">
            </div>
